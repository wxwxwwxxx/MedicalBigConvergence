...
writer = SummaryWriter('/ckpt/medical-beit/debug24_gt_recon_vitae')
...
model2 = ViTAutoEnc(1,128,16).to(device)
...
for epoch in range(100000):
    for i in train_dataloader:
        ...
        output_vl = model2(gt)
        # loss = loss_recon(output_vl[0],gt,i["img_loss_mask"].to(device))
        loss = loss_recon(output_vl[0],gt)
        loss.backward()
        optimizer.step()
        if g_step <= 500:
        ...
        for logi in [0,4,8,11]:
            f0 = output_vl[1][logi]
            c_var = torch.var(torch.mean(f0,1))
            p_var = torch.var(torch.mean(f0,2))
            p_mask = torch.masked_select(f0,i['img_loss_mask'].to(device).view(-1,512)[:,:,None] == 1).view(-1,768)
            c_var_mask = torch.var(torch.mean(p_mask, 0))
            p_var_mask = torch.var(torch.mean(p_mask, 1))
            g_d = torch.abs(model2.get_parameter(f"blocks.{logi}.mlp.linear2.weight").grad).mean()
            g_k = torch.abs(model2.get_parameter(f"blocks.{logi}.attn.qkv.weight").grad).mean()

            writer.add_scalar(f"grad_dense{logi}", g_d, global_step=g_step)
            writer.add_scalar(f"grad_qkv{logi}", g_k, global_step=g_step)
            writer.add_scalar(f"var_channel{logi}", c_var, global_step=g_step)
            writer.add_scalar(f"var_patch{logi}", p_var, global_step=g_step)
            writer.add_scalar(f"var_mask_channel{logi}", c_var_mask, global_step=g_step)
            writer.add_scalar(f"var_mask_patch{logi}", p_var_mask, global_step=g_step)
        if g_step % 10 == 0:
            writer.add_images("img_gt",gt[:, :, 32, :, :], global_step=g_step)
            writer.add_images("recon", output_vl[0][:, :, 32, :, :], global_step=g_step)
